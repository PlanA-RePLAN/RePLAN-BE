...
public class GoalAiService {
  ...
  private String buildRefinePrompt(GoalRefinementRequestDto req, String today) {
    return """
        ...
        6. 목표 달성에 교재·강의가 필요하지만 사용자가 언급하지 않았다면 Google Search로 사용자 수준에 맞는 교재·강의를 검색하여 notes에 추가. 반드시 Google Search로 실제 인터넷에 존재하는지 확인된 것만 추가하며, 확인되지 않은 것은 절대 포함하지 않는다. 책인 경우 저자명·출판사·구매 링크를, 강의인 경우 강사명·플랫폼명·강의 링크를 반드시 함께 명시한다 (예: "해커스 토익 기출 VOCA (저자: 해커스어학연구소 / 출판사: 해커스어학원 / 링크: https://...)", "인프런 - 스프링 핵심 원리 기본편 (강사: 김영한 / 플랫폼: 인프런 / 링크: https://...)")
