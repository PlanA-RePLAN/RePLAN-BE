  ...
  @Operation(
      ...
      description =
          ...
          **비즈니스 로직**
          ...
          2. sub로 사용자 조회 → 없으면 이메일로 조회(있을 때)
             - 기존 애플 회원이면 sub를 채워 넣어 이관
             - 다른 provider로 가입된 이메일이면 409 (토큰 교환 전에 거부)
          3. sub와 이메일 모두로 사용자를 찾을 수 없으면 401 (고아 refresh token 방지를 위해 토큰 교환 전 거부)
          4. authorizationCode를 애플 토큰 엔드포인트에서 refresh token으로 교환(탈퇴 시 철회용)
