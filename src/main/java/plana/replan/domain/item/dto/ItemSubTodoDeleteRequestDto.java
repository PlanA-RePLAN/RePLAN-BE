...
@Schema(
    description =
        """
        예약된 하위 투두 삭제 요청 (행이 아직 없는 회차 전용).
        행이 있는 날짜의 하위 투두는 기존 투두 API(DELETE /api/todos/{parentId}/sub-todos/{subTodoId})를 사용한다.
...
public record ItemSubTodoDeleteRequestDto(
    ...
    @Schema(description = "예약 배열 위치 (상세 응답의 reservedIndex)", example = "0") @NotNull
        Integer index) {}
