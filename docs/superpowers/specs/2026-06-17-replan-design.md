...
- 단일 실패투두 컨텍스트 (리플랜) ← **이번 스펙 범위**
- 지난달 실패패턴 묶음(pastData) (통계 팁노트) ← **다음 스펙**

출력(`summary`+`operations`)은 동일. **이번 스펙은 리플랜용 입력 어댑터까지만** 구현하고,
엔진은 재사용 가능한 형태로 둔다. 통계 팁노트 추천(pastData 입력 + 월간배치/엔드포인트 연동)은
이 엔진을 재사용하는 **별도 후속 스펙**으로 분리한다.
